...
/**
 * Check if a Firebase Storage error means the test environment is unavailable
 * (quota/pricing/access changes), not a functional regression.
 * @param {any} error
 * @returns {boolean}
 */
const isNonFunctionalFirebaseStorageEnvironmentError = (error) => {
  ...
  return (
    message.includes('Cloud Storage for Firebase no longer supports') ||
    message.includes('Quota for bucket')
  );
};
  ...
  describe('Firebase storage', function () {
    ...
    it('uploads an image', async function () {
      ...
      try {
        ...
        if (isNonFunctionalFirebaseStorageEnvironmentError(error)) {
          return;
        }
        throw error;
      }
    ...
    it('gets download url for an image', async function () {
      ...
      let url;
      try {
        url = await promisifyCallbackVariables((callback, result) =>
