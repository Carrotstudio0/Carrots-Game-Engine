...
function formatLayerLabel(rawName) {
	if (!rawName || typeof rawName !== 'string') return '';

	var match = /^\[(.+?)\]\s+(.+)$/.exec(rawName);
	var descriptor = match ? match[2] : rawName;

	for (var i = 0; i < KNOWN_CHANNEL_SUFFIXES.length; i++) {
		var channel = KNOWN_CHANNEL_SUFFIXES[i];
		var suffix = '.' + channel;
		if (descriptor.endsWith(suffix)) {
			var objectName = descriptor.slice(0, descriptor.length - suffix.length).trim();
			if (!objectName) return channel;
			return objectName + ' - ' + channel;
		}
	}
...
function LayerView(layer, dispatcher) {
	...
	*/
	...
	this.setState = function(l, s) {
		...
		var rawName = state.get('name').value;
		label.textContent = formatLayerLabel(rawName);
		label.title = rawName;
