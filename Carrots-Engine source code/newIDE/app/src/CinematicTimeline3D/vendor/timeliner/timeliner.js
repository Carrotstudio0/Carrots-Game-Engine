...
function Timeliner(target, options) {
	options = options || {};
	...
	var onPlaybackChanged = typeof options.onPlaybackChanged === 'function' ? options.onPlaybackChanged : null;
	var onTargetNotify = typeof options.onTargetNotify === 'function' ? options.onTargetNotify : null;
	...
	function emitDataChanged(reason) {
		...
		try {
			onDataChanged(JSON.parse(data.getJSONString()), reason);
		} catch (error) {
			console.warn('Timeliner failed to emit changed data.', error);
		}
	}

	function emitPlaybackChanged(isPlaying) {
		if (!onPlaybackChanged) return;
	...
	function setCurrentTime(value) {
		...
		if (onTimeChanged) onTimeChanged(value);
	}

	dispatcher.on('target.notify', function(name, value) {
		if (target) target[name] = value;
		if (onTargetNotify) onTargetNotify(name, value);
	});
	...
	*/
	...
	function repaintAll() {
		var content_height = Math.max(layers.length * Settings.LINE_HEIGHT, 1);
	...
	*/
	...
	style(pane, {
		...
		top: isEmbedded ? '0px' : '20px',
		left: isEmbedded ? '0px' : '20px',
	...
	*/
	...
	function resize(width, height) {
		width = Math.max(Number(width) || 0, 320);
		height = Math.max(Number(height) || 0, 180);
	...
	function ensureLayer(name) {
		return getLayerByName(name) || addLayer(name);
	}

	function setLayerValueAtCurrentTime(name, value, dont_save) {
		var layer = ensureLayer(name);
		if (!layer) return;
		dispatcher.fire('value.change', layer, value, !!dont_save);
	}

	this.addLayer = addLayer;
	this.ensureLayer = ensureLayer;
	this.getLayer = getLayerByName;
	this.getLayers = function() {
		return layer_store.value;
	};
	this.setLayerValueAtCurrentTime = setLayerValueAtCurrentTime;
	this.setCurrentTime = setCurrentTime;
	this.getCurrentTime = function() {
		return data.get('ui:currentTime').value;
	};
	...
	this.getData = function() {
		return JSON.parse(data.getJSONString());
	};
	this.getJSONString = function(format) {
		return data.getJSONString(format);
	};
	this.resize = resize;
	...
	this.dispose = function dispose() {
		...
		if (keydownListener && !disableGlobalKeybindings) {
			document.removeEventListener('keydown', keydownListener);
		}

		var domParent = pane.parentElement;
		if (domParent) domParent.removeChild(pane);

		var ghostPaneParent = ghostpane.parentElement;
		if (ghostPaneParent) ghostPaneParent.removeChild(ghostpane);

		var rootParent = root.parentElement;
		if (rootParent) rootParent.removeChild(root);
	...
	if (!isEmbedded) {
		...
		style(top_right_bar, { display: 'none' });
	}
	...
	if (isEmbedded) {
		...
		var initialWidth = container.clientWidth || 960;
		var initialHeight = container.clientHeight || 420;
